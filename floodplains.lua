...
-- We assume engine.filterCutoff and engine.filterRQ are provided by GlutXtd.
...
local function randomize_voice(i)
  ...
  if i <= num_voices then
    -- For voices 1..5, we still do the "smooth_transition" on the param system
    ...
    -- For voices 6..7, skip param calls. Directly call engine methods:
    engine.jitter(i,  new_jitter / 1000)
    engine.size(i,    new_size   / 1000)
    engine.density(i, new_density)
    engine.spread(i,  new_spread / 100)
  end
end
...
local function setup_engine()
  for i = 1, num_voices do
    -- disable automatic playhead advancement
    engine.gate(i, 0)
    engine.speed(i, 0)
  end
  -- Also set aux voices speed=0
  engine.gate(6, 0)
  engine.speed(6, 0)
  engine.gate(7, 0)
  engine.speed(7, 0)
...
local function envelope_attack(i)
  ...
  envelope_threads[i] = clock.run(function()
    ...
    local target_vol = params:get(
      (i>num_voices) and params:get("polyphonic_voice").."volume" or (i.."volume")
    )
    local target_pan = params:get(
      (i>num_voices) and params:get("polyphonic_voice").."pan" or (i.."pan")
    )
      ...
      for i = 1, num_voices do
        if msg.ch == params:get("midi_channel_"..i) then
          if i == params:get("polyphonic_voice") then
            ...
            voice_active[i] = true
            -- Do not re-read sample so the playhead isn't reset
            if #chord == 1 then
              engine.pitch(i, math.pow(2, (chord[1]-60)/12))
              engine.gate(i, 1)
              envelope_attack(i)
...
---
--- Key-handling for randomizing voices
---
function key(n, z)
  if n == 1 then
    if z == 1 then
